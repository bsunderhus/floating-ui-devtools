import { CONTROLLER, ELEMENT_METADATA } from '@floating-ui-devtools/core';
  ...
  const controller: Controller = {
    ...
    select: (nextSelectedElement: HTMLElement | null) => {
      if (isHTMLElementWithMetadata(nextSelectedElement)) {
        ...
        observer.observe(nextSelectedElement.parentElement, { childList: true, subtree: false });
      }
...
const isHTMLElementWithMetadata = (
  ...
  Boolean(element && ELEMENT_METADATA in element && element.parentElement !== null);
